...
select animals.name, vets.name, visits.date_of_visit from animals inner join visits on animals.id=visits.animals_id
...
select count(animals.name), vets.name from animals inner join visits on animals.id=visits.animals_id
inner join vets on visits.vets_id=vets.id
...
SELECT COUNT(visits.date_of_visit)-(SELECT COUNT(vets.name) 
